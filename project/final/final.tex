...
We evaluate effectiveness of the access primitives by comparing with the baseline performance. For the purposes of the evaluation, we assume that performance behavior of new primitives comparable to traditional row major access pattern so we use row major access pattern as the baseline for our evaluation.
...
\begin{figure}[t]
	...
	\includegraphics[scale=0.35]{images/128_cache}
        \label{fig:eval_cache}
    }
    \subfloat[Runtime]{
	\includegraphics[scale=0.35]{images/128_runtime}
        \label{fig:eval_runtime}
    }
    \caption{
        \textbf{Performance} -- Evaluation of workload
    }
